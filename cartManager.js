...
import { Carrito } from "./server/models/carrito.model.js";
import { productManager } from "./productManager.js";
class CartManager {
  constructor() {
    this.filename = "carritosDB.json";
    this.carritosDB = [];
    fsp.access(this.filename)
      ...
      .then((data) => {
        this.carritosDB = JSON.parse(data);
      })
      .catch((err) => {
        return fsp.writeFile(this.filename, JSON.stringify(this.carritosDB));
      });
  }
  getAllCarritos() {
    return this.carritosDB;
  }
  getById(id){
    const getCarr = this.carritosDB.find(carr => carr.id == id)
    getCarr.products = getCarr.products.map(eachProduct => {
      const producto = productManager.getById(eachProduct.id);
      if (!producto.id) return eachProduct;
      return {
        ...producto,
  ...
  save() {
    fsp.writeFile(this.filename, JSON.stringify(this.carritosDB));
  }
  createCarrito(productos) {
    const nuevoCarro = new Carrito();
    nuevoCarro.createCarro(productos)
    this.carritosDB.push(nuevoCarro);
    this.save();
  }
  putCart(newCart){
    this.save();
  ...
  deleteCart(cid) {
    const allCarts = this.carritosDB.filter(oneCart => oneCart.id != cid);
    this.carritosDB = allCarts
    this.save();
  }
}
